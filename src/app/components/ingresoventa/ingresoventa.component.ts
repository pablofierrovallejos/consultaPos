...
export class IngresoventaComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.registrarAuditoriaConsultaIngresoVenta();
  }

  async registrarAuditoriaConsultaIngresoVenta(): Promise<void> {
    try {
      const geoData = await this.obtenerGeolocalizacion();
      const auditoria = {
        hostorigen: geoData.query || 'localhost',
        modulo: '/ingresoventa',
        accionrealizada: 'Consulta Ingreso Venta',
        usuario: 'hp',
        detalles: `Acceso al formulario de ingreso de ventas | Precisión: ${geoData.precision} | ISP: ${geoData.isp}`,
        dataprocesada: JSON.stringify({
          fecha_acceso: new Date().toISOString(),
          url: window.location.href,
          geolocalizacion: { precision: geoData.precision, isp: geoData.isp, userAgent: geoData.userAgent }
        }),
      ...
      this.api.registrarAuditoria(auditoria).subscribe(
        () => console.log('✅ Auditoría ingreso venta registrada'),
        (error) => console.warn('⚠️ Error auditoría ingreso venta:', error)
      );
    } catch (error) {
      console.error('❌ Error auditoría ingreso venta:', error);
    }
  }
